...
Public Sub StartEngine()
  ...
  Select Case Result
    Case NO_MATE
      ...
      If UCIMode Then
          If pbPonderMode And PonderMove.From <> 0 Then
            If Pondertrace Then WriteTrace "Send to GUI ponder: " & "bestmove" & " " & sCoordMove & " ponder " & CompToCoord(PonderMove)
            SendCommand "bestmove" & " " & sCoordMove & " ponder " & CompToCoord(PonderMove)
            MovePondering = PonderMove
          Else
            If Pondertrace Then WriteTrace "Send to GUI normal: " & "bestmove" & " " & sCoordMove
  ...
  If BookMovePossible Then
   ...
   End If
    ...
    For RootDepth = 1 To IMax
      ...
      ' easy move?
      If RootDepth >= 7 And EasyMove.From > 0 And Not FixedDepthMode And Not FixedTime > 0 And Not pbPondering Then
      ...
      If RootDepth > 20 Then  ' emergency exit or mate found?
        If RootDepth > 80 Or (Abs(FinalScore) > MATE0 - 6 And Abs(FinalScore) < MATE0) And Not pbPondering Then bTimeExit = True
      End If
      
      If pbPondering And bTimeExit Then bTimeExit = False
...
lblNextRootDepth:
      If ThreadNum > 0 Then If ReadMainThreadStatus() = 0 Then bTimeExit = True: pbPondering = False: Exit For
...
lblIterationsExit:
    ...
    If pbPonderMode Then
      PonderMove = GetPonderMove()
    End If
    
    If Pondertrace Then WriteTrace ">>>>>THINK End: Ponder: " & pbPonderMode & " Pondering: " & pbPondering & ", Move: " & CompToCoord(PonderMove)
    
    If bTimeTrace Then WriteTrace "Think: end : " & MoveText(Think) & " " & Now()
     
  End Function '<<<<<<<< end of THINK <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<

Public Function GetPonderMove() As TMOVE
   Static FinalMoveForPonder As TMOVE
   Static LastPonderMove As TMOVE
   
   If PVLength(1) - 1 >= 1 Then
      ' Save Hint move
        If Not MovesEqual(FinalMoveForPonder, PV(1, 1)) Then
   '       If Pondertrace Then WriteTrace "GetPonderMove1: not equal " & CompToCoord(FinalMoveForPonder) & " - " & CompToCoord(PV(1, 1))
          ClearMove FinalMoveForPonder  ' for case that 1. ply as hash move only
          ClearMove LastPonderMove
        End If
   End If
   If PVLength(1) - 1 >= 2 Then
        If PV(1, 2).From > 0 Then LastPonderMove = PV(1, 2): FinalMoveForPonder = PV(1, 1)
   '     If Pondertrace Then WriteTrace "GEtPonderMove2 Final: " & CompToCoord(FinalMoveForPonder) & " - Ponder:" & CompToCoord(LastPonderMove)
   End If
    
    GetPonderMove = LastPonderMove
End Function
...
lblNoMoreReductions:
    ...
    '--- 18. Unmake move
    '-------------------
    Call RemoveEpPiece: Ply = Ply - 1: UnmakeMove CurrentMove
   ...
   'If bTimeTrace Then WriteTrace "SearchRoot: FixedTime: " & FixedTime & " " & FixedDepthMode & ", TimeDiff:" & TimeElapsed()
    If Not FixedDepthMode And GoodMoves > 0 And Not bAnalyzeMode Then
      If FixedTime > 0 Then
        If TimeElapsed() >= FixedTime - 0.1 And Not pbPondering Then
          bTimeExit = True
        End If
      ElseIf (RootDepth > LIGHTNING_DEPTH) And Not pbPondering Then ' Time for next move?
...
lblEndRootMoves:
  ...
  If LegalMoveCnt = 0 Then ' no move
    ...
    GoodMoves = -1
  Else
    If (LegalMoveCnt = 1 And RootDepth = 1) And Not bTimeExit Then bOnlyMove = True: RootScore = 0: FinalScore = 0: pbPondering = False ' single move only?
...
Private Function Search(ByVal ss As Long, _
 ...
 ' If Alpha < -DrawContemptForSide() And Fifty >= 3 Then
  ...
  ClearMove ttMove: ttValue = VALUE_NONE: bTTCapture = False
  ...
  If Not FixedDepthMode Or ThreadNum > 0 Then
    ...
    If (Nodes > LastNodesCnt + (GUICheckIntervalNodes * 2 \ (1 + Abs(bEndgame)))) And (RootDepth > LIGHTNING_DEPTH Or Ply = 2) Then
      ...
      If FixedTime > 0 Then
        If Not bAnalyzeMode And TimeElapsed() >= FixedTime - 0.1 And Not pbPondering Then bTimeExit = True: Exit Function
      ElseIf Not bAnalyzeMode And Not pbPondering Then
      ...
      If Not (ttHit And HashDepth >= Depth - 3 And ttValue <> VALUE_NONE And ttValue < rBeta) Then '+++2023+++
        ...
        Do While MovePicker(ss, CurrentMove, LegalMovesOutOfCheck)
          If CurrentMove.Captured <> NO_PIECE Or CurrentMove.Promoted > 0 Then
              ...
              MakeMove CurrentMove: Ply = Ply + 1
              bLegalMove = False
              If CheckLegal(CurrentMove) Then
...
lblMovesLoop:
 ...
 If bSingularExtensionNode Then
   ...
   If MovePossible(ttMove) Then
      ...
      '--- Make move            -
      MakeMove ttMove: Ply = Ply + 1
      bLegalMove = CheckLegal(ttMove)
      '--- Undo move ------------
      Call RemoveEpPiece: Ply = Ply - 1: UnmakeMove ttMove
    ...
    If Score > BestValue And bLegalMove Then
      ...
      If (Score > Alpha) Then
        ...
        If PVNode And Score < Beta Then
          Alpha = Score
          If Depth > 1 And Depth < 6 And Beta < 10500 And Alpha > -10500 Then Depth = Depth - 1
...
'  If Alpha >= Beta Then QSearch = Alpha: Exit Function
  ...
  BestValue = -VALUE_INFINITE: StaticEval = VALUE_NONE: SetMove BestMove, EmptyMove
  ...
  Do While MovePicker(ss, CurrentMove, LegalMovesOutOfCheck)
    ...
    '------------------
    CmhPtr(ss) = CurrentMove.Piece * MAX_BOARD + CurrentMove.Target
    MakeMove CurrentMove: Ply = Ply + 1: bLegalMove = False
...
Public Sub ClearEasyMove()
  Dim i As Long
  For i = 1 To 3: ClearMove EasyMovePV(i): Next
  EasyMoveStableCnt = 0
End Sub
