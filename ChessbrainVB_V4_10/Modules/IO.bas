...
Function PollCommand() As Boolean
  ...
  Else
    ...
    Select Case MainThreadStatus
      ...
      Case 0
        If LastThreadStatus <> MainThreadStatus Then
          ThreadCommand = "exit" & vbLf: PollCommand = True: bTimeExit = True: pbPondering = False
...
Public Sub SendThinkInfo(Elapsed As Single, ActDepth As Long, CurrentScore As Long, Alpha As Long, Beta As Long)
  ...
  Dim sPost               As String, j As Long, sPostPV As String
    ...
    If UCIMode Then
      ...
      sPost = sPost & " nodes " & CalcNodes() & " nps " & CalcNPS(Elapsed) & " tbhits " & EGTBasesHitsCnt & " time " & Int(Elapsed * 1000#)
    Else
    ...
    If pbPonderMode Then GetPonderMove ' Update FinalMoveForPonder
    
    If Trim$(sPostPV) <> "" Then sPost = sPost & " pv" & sPostPV
