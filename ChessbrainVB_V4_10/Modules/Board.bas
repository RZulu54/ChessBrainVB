...
Private bGenQuiets                                As Boolean ' generate non-captures
Private bGenQuietChecksOnly                       As Boolean ' in QS: only quiet checking moves
'------------------------------------
...
Public Function GenerateMoves(ByVal Ply As Long, _
  ...
  bGenQuiets = Not bGenCapturesOnly
  bGenQuietChecksOnly = (bGenCapturesOnly And bGenQsChecks)
  If bWhiteToMove Then

    For i = WhitePiecesStart To WhitePiecesEnd
      From = Pieces(i)
      If From <> 0 Then ' from=0 if piece was captured during search
...
Private Sub TryMoveWPawn(ByVal Ply As Long, _
                              NumMoves As Long, _
  ...
  If PieceTarget = BKING Then Exit Sub
  ...
  Else
    With Moves(Ply, NumMoves)
      ...
      Case NO_PIECE, WEP_PIECE ' WEP_PIECE should not appear
        ...
        bDoCheckMove = False
        If bGenQuiets Then
          ' normal search: include all quiets
        ElseIf bGenQuietChecksOnly Then
          ' qsearch first ply: include only quiet checks
          bDoCheckMove = IsCheckingMove(PieceFrom, From, Target, 0, 0)
        End If
        If bGenQuiets Or bDoCheckMove Then
...
Private Sub TryMoveBPawn(ByVal Ply As Long, _
                              NumMoves As Long, _
                              ByVal From As Long, _
                              ByVal Target As Long)
  If Board(Target) = FRAME Then Exit Sub
  ...
  PieceFrom = Board(From): PieceTarget = Board(Target)
  Debug.Assert PieceTarget <> FRAME
  If PieceTarget = WKING Then Exit Sub
...
Private Sub TryMoveListKnight(ByVal Ply As Long, _
                                   NumMoves As Long, _
                                   ByVal From As Long)
  '--- Knights only moves
  ...
  For ActDir = 0 To 7
    ...
    Case Else
      ...
      If PieceCol <> (PieceTarget And 1) Then ' Capture of own piece not allowed
        If PieceTarget = WKING Or PieceTarget = BKING Then Exit Sub
        With Moves(Ply, NumMoves)
          .From = From: .Target = Target: .Piece = PieceFrom: .IsLegal = False: .IsChecking = False: .EnPassant = 0: .Castle = NO_CASTLE: .Captured = PieceTarget: .CapturedNumber = 0: .Promoted = 0: .SeeValue = VALUE_NONE: .OrderValue = 0
        End With
        NumMoves = NumMoves + 1
...
Private Sub TryMoveListKing(ByVal Ply As Long, _
                                 NumMoves As Long, _
                                 ByVal From As Long)
  '--- Kings only
...
Private Sub TryCastleMove(ByVal Ply As Long, _
                               NumMoves As Long, _
                               ByVal From As Long, _
                               ByVal Target As Long)
  If Board(Target) = FRAME Then Exit Sub
  ...
  For ActDir = DirStart To DirEnd ' for all possible directions
      ...
      Do While Board(Target) <> FRAME '--- Slide loop
        ...
        Else
          bDoCheckMove = False
        End If
        If bGenQuiets Or bDoCheckMove Then '---Normal move, not generated in QSearch (exception: when in check)
...
lblExit:
  ...
  If EpPosArr(Ply) > 0 Then Board(EpPosArr(Ply)) = IIf(EpPosArr(Ply) < SQ_H4, WEP_PIECE, BEP_PIECE)  ' ResetEpPiece
...
Public Sub ResetEpPiece()
  ...
  If EpPosArr(Ply) <> 0 Then
    If EpPosArr(Ply) < SQ_H4 Then Board(EpPosArr(Ply)) = WEP_PIECE Else Board(EpPosArr(Ply)) = BEP_PIECE
  End If
End Sub
...
lblContinue:
  ...
  For i = 1 To Cnt
    If Blocker(i) >= 0 Then
      If PieceList(i) > 0 Then
        NumAttackers(COL_WHITE) = NumAttackers(COL_WHITE) + 1
      Else
        NumAttackers(COL_BLACK) = NumAttackers(COL_BLACK) + 1
      End If
    End If
  Next
